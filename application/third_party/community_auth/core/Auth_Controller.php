...
class Auth_Controller extends CI_Controller {
	...
	protected function _set_user_variables()
	{
		...
		$this->auth_NIK  	 = $this->auth_data->NIK;
		...
		$this->config->set_item( 'auth_department',    $this->auth_department );
		$this->config->set_item( 'auth_dimension',    $this->auth_dimension );
	...
	// -----------------------------------------------------------------------
	// custom for dept.
	protected function get_department( $str )
	{
		$query = $this->db->select('m_group.group_name')
						  ->from('users_position')
						  ->join('m_title','m_title.title_id = users_position.title_id','left')
						  ->join('m_group','m_group.group_id = m_title.group_id','left')
						  ->where('users_position.user_id',$str)
						  ->limit(1)
						  ->get()
						  ->row();
	...
	protected function get_dimension($str)
	{
		$que = $this->db->select('dimension_group.dg_Code')
		                ->from('dimension_group')
						->join('m_title','m_title.group_id = dimension_group.group_id','left')
						->join('users_position','users_position.title_id = m_title.title_id','left')
						->where('users_position.user_id',$str)
						->get();
